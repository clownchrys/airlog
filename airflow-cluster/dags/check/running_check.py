...
dag = DAG(
    ...
    concurrency=2,  # 동시 실행가능한 Task 인스턴스의 수 (default: 16)
...
with dag:
    ...
    op_check1 = python.PythonOperator(
        task_id="op_check1",
        python_callable=fn,
        op_args=[],
        op_kwargs={
            "sleep": 2,
            "msg": "Completed: check1"
        }
    )

    op_check2 = python.PythonOperator(
        task_id="op_check2",
        python_callable=fn,
        op_args=[],
        op_kwargs={
            "sleep": 5,
            "msg": "Completed: check2"
        }
    )

    op_cleanup = python.PythonOperator(
        task_id="op_cleanup",
        python_callable=fn,
        op_args=[],
        op_kwargs={
            "sleep": 0,
            "msg": "Completed: cleanup"
        }
    )

op_init >> [op_check1, op_check2] >> op_cleanup
